...
class TestDataPipelineService:
    ...
    def test_load_config_success(self, mock_config):
        ...
        with patch('data_pipeline.main.load_secure_config') as mock_load:
    ...
    async def test_main_async_config_validation_failure(self):
        ...
        with patch('data_pipeline.main.load_config', return_value=mock_config), \
             patch('data_pipeline.main.validate_config', return_value=(False, ["error"], [])):
    ...
    async def test_main_async_service_initialization_failure(self, mock_config):
        ...
        with patch('data_pipeline.main.load_config', return_value=mock_config), \
             patch('data_pipeline.main.validate_config', return_value=(True, [], [])), \
             patch('data_pipeline.main.setup_logging'), \
             patch('data_pipeline.main.ServiceManager'), \
             patch('data_pipeline.main.EnhancedHealthMonitor'), \
             patch('data_pipeline.main.DataPipelineService', side_effect=Exception("Init failed")):
